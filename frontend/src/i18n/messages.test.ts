...
describe('i18n', () => {
  ...
  it('translates keys with English fallback', () => {
    ...
    expect(translate('footer.previousTickets', 'es')).toBe('Tickets anteriores')
    expect(translate('closedTickets.cardNotReimbursable', 'en')).toMatch(/card payment/i)
    expect(translate('closedTickets.reimburse', 'es')).toBe('Reembolsar')
